 ...
 *
 * 用法：
 *   - StoreSearchScreen 选中搜索结果时调用 `requestFocus(store)` 再 goBack 到
 *     首页「买手店」Tab 里的地图形态。
 *   - BuyerMapScreen 订阅这个 store；`pending` 一变成非空就消费掉，把地图
 *     animateToRegion 到该店并打开 callout。
 *
 * 为什么不直接用 react-navigation 的 `route.params`：
 * BuyerMapScreen 是 Discover「买手店」Tab 内部的嵌入子页，不是独立路由，
 * 自身不会随 navigate 收到 focusEffect / 新 params。用一个 pub-sub store
 * 把信号脱钩出来最简单，并且天然支持"未挂载先压入、挂载后 consume"。
 *
