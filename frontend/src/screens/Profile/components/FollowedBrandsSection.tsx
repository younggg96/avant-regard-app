...
interface FollowedBrandsSectionProps {
  brands: FollowingBrand[];
}

export const FollowedBrandsSection = ({ brands }: FollowedBrandsSectionProps) => {
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable
            ...
            onPress={() => navigation.navigate("BrandDetail", { name: item.name })}
          >
            {item.coverImage ? (
              <OptimizedImage
                uri={item.coverImage}
