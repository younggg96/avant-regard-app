...
export function DownloadCTAs({ className, variant = "default" }: DownloadCTAsProps) {
  /*
   * variant="inverted" is used on the dark CTA section (bg-ink).
   * In dark mode body, the "default" variant also inverts because .btn-primary
   * flips to white bg / black text.
   */
  const primaryClass =
    variant === "inverted"
      ? "inline-flex items-center gap-3 rounded border border-white/15 bg-white px-5 py-3 font-label text-sm font-medium text-black transition-all duration-200 hover:bg-[#e8e8e8] active:scale-[0.98]"
      : "btn-primary inline-flex items-center gap-3 px-5 py-3 text-sm";

  const secondaryClass =
    variant === "inverted"
      ? "inline-flex items-center gap-3 rounded border border-white/20 bg-transparent px-5 py-3 font-label text-sm font-medium text-white transition-all duration-200 hover:bg-white/10 active:scale-[0.98]"
      : "btn-secondary inline-flex items-center gap-3 px-5 py-3 text-sm";
    ...
    <div className={`flex flex-wrap items-center gap-3 ${className ?? ""}`}>
      <Link href={config.appStoreUrl} className={primaryClass} rel="noopener">
        ...
        <span className="flex flex-col items-start leading-tight">
          <span className="text-[9px] uppercase tracking-[0.18em] opacity-55">Download on the</span>
          <span className="text-sm tracking-wide">App Store</span>
        </span>
      </Link>

      <Link href={config.playStoreUrl} className={secondaryClass} rel="noopener">
        <span className="opacity-75">{ICON_PLAY}</span>
        <span className="flex flex-col items-start leading-tight">
          <span className="text-[9px] uppercase tracking-[0.18em] opacity-55">Get it on</span>
